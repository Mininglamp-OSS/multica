...
| `S3_KEY_PREFIX` | 空 | 可选的对象 key 前缀（例如 `multica-prod`），用于多个应用共用同一个 bucket/CDN 域名时做目录隔离。首尾斜杠会被自动去掉。留空则保持现有不带前缀的 key。**请在第一次上传前就设置好**——上线后再修改不会迁移已有对象，会导致已有附件下载报 404、删除静默失效，除非手动把它们复制到新前缀目录下 |
