...
| `AWS_ENDPOINT_URL` | Custom S3-compatible endpoint (e.g. MinIO, R2, B2, Tencent COS). Setting this switches to path-style URLs by default — see `S3_FORCE_PATH_STYLE` |
| `S3_FORCE_PATH_STYLE` | Defaults to `true` when `AWS_ENDPOINT_URL` is set. Set to `false` for backends that require virtual-hosted-style requests, e.g. **Tencent COS**, which rejects path-style with `PathStyleDomainForbidden`. No effect unless `AWS_ENDPOINT_URL` is also set |
