...
| `S3_KEY_PREFIX` | 비어 있음 | 선택적 객체 key 접두사 (예: `multica-prod`). 하나의 버킷/CDN 도메인을 다른 애플리케이션과 공유할 때 유용합니다. 앞뒤 슬래시는 자동으로 제거됩니다. 비워두면 기존의 접두사 없는 key가 그대로 유지됩니다. **첫 업로드 전에 반드시 설정하세요** — 나중에 변경하면 기존에 업로드된 객체는 마이그레이션되지 않아 다운로드는 404가 되고 삭제는 조용히 무시됩니다. 새 접두사 아래로 직접 복사하지 않는 한 복구되지 않습니다 |
