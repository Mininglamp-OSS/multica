...
| `S3_KEY_PREFIX` | 空 | オプションのオブジェクト key プレフィックス（例: `multica-prod`）。1 つのバケット/CDN ドメインを他のアプリケーションと共有する場合に便利です。先頭・末尾のスラッシュは自動的に取り除かれます。空のままにすると既存のプレフィックスなしの key が維持されます。**必ず最初のアップロード前に設定してください** — 後から変更しても既存のオブジェクトは移行されないため、既存の添付ファイルはダウンロードが 404 になり、削除は静かに失敗します。手動で新しいプレフィックス配下にコピーしない限り復旧しません |
